...
class AsyncMessageProcess {
   ...
   private:
    ...
    size_t max_size_ = 2000;  // 增大队列防止关键帧丢失导致点云未优化
    std::string name_;
...
void AsyncMessageProcess<T>::AddMessage(const T& msg) {
    ...
    while (msg_buffer_.size() >= max_size_) {
        AWARN << name_ << " queue full (" << max_size_ << "), waiting...";
        lock.unlock();
...
void AsyncMessageProcess<T>::Quit() {
    // 先处理完队列中剩余的消息，再退出
    {
        ...
        if (!msg_buffer_.empty()) {
            AINFO << name_ << " processing remaining " << msg_buffer_.size() << " messages before quit...";
            auto buffer = msg_buffer_;
            msg_buffer_.clear();
            lock.unlock();
            ...
            AINFO << name_ << " finished processing remaining messages";
        }
    }
