...
// Functions moved to common/math_utils.h are removed from here to avoid duplication
// SKEW_SYM_MATRIX, Exp, Log, RotMtoEuler, RpyToRotM2, VecFromArray, MatFromArray, Mat4FromArray
// rad2deg, deg2rad, limit_in_range, KeepAngleInPI
...
inline PoseRPYD SE3ToRollPitchYaw(const SE3& pose) {
    ...
    double cy = std::sqrt(rot(0, 0) * rot(0, 0) + rot(1, 0) * rot(1, 0));
    bool singular = cy < 1e-6;
    if (!singular) {
        output.roll = std::atan2(rot(2, 1), rot(2, 2));
        output.pitch = std::atan2(-rot(2, 0), cy);
...
inline bool PoseInterp(double query_time, C&& data, FT&& take_time_func, FP&& take_pose_func, SE3& result,
    ...
    if (data.size() <= 1) {
        if (verbose) {
            AINFO << "data size is too small for interp: " << data.size();
        }
        return false;
    }
    ...
    if (query_time > last_time) {
        if (verbose) {
            AWARN << "query time is later than last time.";
        }
        ...
        if (verbose) {
            AWARN << "pose interp failed: " << query_time << ", " << (last_time + time_th) << ", "
        ...
        if (query_time < (first_time - time_th)) {
            if (verbose) {
                AWARN << "query time too early: " << ", " << query_time << ", " << (first_time - time_th);
            }
            return false;
        } else {
