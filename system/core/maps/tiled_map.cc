...
bool TiledMap::ConvertFromFullPCD(CloudPtr map, const SE3& start_pose, const std::string& map_path) {
    ...
    for (const auto& pt : map->points) {
        // 过滤 NaN 或 Inf 点，避免 Pos2Grid 产生 INT_MIN 坐标
    ...
    if (nan_count > 0) {
        AWARN << "[TiledMap] Filtered " << nan_count << " NaN/Inf points during map conversion";
    }
...
void TiledMap::SaveToBin(bool only_dynamic) {
    AINFO << "map is saved to " << options_.map_path_ << ", sz: " << static_chunks_.size();
...
bool TiledMap::LoadMapIndex() {
    ...
    if (!fin) {
        AERROR << "cannot load map index from: " << options_.map_path_;
        return false;
    }
        ...
        if (reading_fp) {
            ...
            if (options_.policy_ == DynamicCloudPolicy::PERSISTENT && options_.load_dyn_cloud_) {
                ...
                if (PathExists(dyn_filename)) {
                    AINFO << "load dynamic chunk: " << dyn_filename;
    ...
    AINFO << "loaded chunks: " << static_chunks_.size() << ", fps: " << func_points_.size();
    ...
    while (!fin.eof()) {
        ...
        if (iter == dynamic_polygon_.end()) {
            ...
            iter->second.polygon_.emplace_back(Vec2f(pos[0], pos[1]));
        }
    }
    AINFO << "dynamic polygons: " << dynamic_polygon_.size();
...
void TiledMap::LoadOnPose(const SE3& pose) {
    ...
    for (const auto& cp : static_chunks_) {
        ...
        if (n <= options_.load_map_size_) {
            ...
            if (dyn_iter == dynamic_chunks_.end()) {
                // 创建这个动态区块（如果配置允许）
                if (options_.policy_ == DynamicCloudPolicy::PERSISTENT || options_.load_dyn_cloud_) {
                    std::string dyn_path = options_.map_path_ + "/" + std::to_string(cp.second->id_) + "_dyn.pcd";
                    if (PathExists(dyn_path)) {
                        auto new_chunk = std::make_shared<MapChunk>(cp.second->id_, cp.first, dyn_path);
    ...
    // AINFO << "static_grids_ size is: " << static_grids_.size()
...
void TiledMap::ResetDynamicCloud() {
    ...
    if (options_.policy_ == DynamicCloudPolicy::PERSISTENT) {
        ...
        for (auto it = id_to_grid_.begin(); it != id_to_grid_.end(); it++) {
            ...
            std::string dyn_filename = options_.map_path_ + "/" + std::to_string(id) + "_dyn.pcd";
            if (PathExists(dyn_filename)) {
                AINFO << "reset dynamic chunk: " << dyn_filename;
                auto dyn_chunk = std::make_shared<MapChunk>(id, grid, dyn_filename);
                dynamic_chunks_.emplace(grid, dyn_chunk);
        ...
        if (init_dy_size > 0) {
            AINFO << "reset dynamic map success, load size: " << init_dy_size;
        }
    }
...
static bool IsPointInPolygon(const std::vector<Vec2f>& polygon, const Vec2f& point) {
    bool inside = false;
...
int TiledMap::FallsInDynamicArea(const Vec3f& pt) {
    ...
    for (auto& pp : dynamic_polygon_) {
        ...
        if (IsPointInPolygon(pp.second.polygon_, Vec2f(pt[0], pt[1]))) {
            return pp.first;
        }
    }
