...
#ifndef AIR_LOCALIZATION_G2P5_H
#define AIR_LOCALIZATION_G2P5_H
...
#include "modules/air_mapping/system/core/system/async_message_process.h"
...
class G2P5 {
   public:
    ...
    /// 根据关键帧列表重新生成带射线清除的栅格地图 (同步执行，用于保存)
    void GenerateMapFromKeyframes(const std::vector<Keyframe::Ptr>& keyframes);
    ...
    /// 保存地图为 PPM 文件
    bool SaveMapPPM(const std::string& filename);
    
    /// 保存地图为 PGM 文件
    bool SaveMapPGM(const std::string& filename);
