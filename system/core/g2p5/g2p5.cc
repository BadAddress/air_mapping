...
#include "modules/air_mapping/system/core/g2p5/g2p5.h"
#include "modules/air_mapping/system/common/constant.h"
...
#include "modules/air_mapping/system/utils/timer.h"
...
void G2P5::RenderBack() {
    while (!quit_flag_) {
        ...
        unsigned long cur_idx = all_keyframes.back()->GetID();
        while (true) {
            ...
            unsigned long frontend_idx = frontend_kf->GetID();
            std::vector<Keyframe::Ptr> kfs;

            {
                UL lock(kf_mutex_);
                // 安全检查：防止访问越界
                if (frontend_idx >= all_keyframes_.size()) {
                    LOG(WARNING) << "Frontend KF index " << frontend_idx 
                                 << " exceeds all_keyframes_ size " << all_keyframes_.size();
                    frontend_idx = all_keyframes_.size() - 1;
                }
                
                if (cur_idx < frontend_idx) {
                    for (unsigned long i = cur_idx + 1; i <= frontend_idx; ++i) {
                         if (i < all_keyframes_.size()) {
...
void G2P5::GenerateMapFromKeyframes(const std::vector<Keyframe::Ptr>& keyframes) {
    if (keyframes.empty()) {
        LOG(WARNING) << "No keyframes to generate map.";
        return;
    }

    LOG(INFO) << "Generating map from " << keyframes.size() << " keyframes with ray casting (clearing)...";
    ...
    G2P5Map::Options opt;
    opt.resolution_ = options_.grid_map_resolution_;
    auto map = std::make_shared<G2P5Map>(opt);

    // 批量添加关键帧（包含 Resize 和 Convert3DTo2DScan）
    // Convert3DTo2DScan 内部会执行 SetWhitePoints (射线清除) 和 SetHitPoint
    AddKfToMap(keyframes, map);

    {
        UL lock{newest_map_mutex_};
        newest_map_ = map;
        // 同时更新前端地图，防止后续如果不退出继续跑的话出现不一致
        frontend_map_ = map;
    }
...
bool G2P5::ResizeMap(const std::vector<Keyframe::Ptr> &kfs, G2P5MapPtr &map) {
    ...
    for (auto it = kfs.begin(); it != kfs.end(); ++it) {
        ...
        SE3 T_imu_lidar(state.offset_R_lidar_, state.offset_t_lidar_);
        // 计算完整变换 T_world_lidar = T_world_imu * T_imu_lidar
        SE3 T_world_lidar = pose * T_imu_lidar;
        ...
        for (size_t i = 0; i < cloud->points.size(); i += 10) {
            ...
            Vec3d point = T_world_lidar * cloud->points[i].getVector3fMap().cast<double>();
...
void G2P5::Convert3DTo2DScan(Keyframe::Ptr kf, G2P5MapPtr &map) {
    ...
    SE3 T_imu_lidar(state.offset_R_lidar_, state.offset_t_lidar_);
    // 计算完整变换 T_world_lidar = T_world_imu * T_imu_lidar
    SE3 T_world_lidar = Twb * T_imu_lidar;
    ...
    for (size_t i = 0; i < cloud->points.size(); ++i) {
        ...
        if (dis_floor > min_th) {
            if (dis_floor < max_th) {
                ...
                /// 设置黑点 (使用完整的 T_world_lidar 变换)
                Vec3d p_world = T_world_lidar * pc;
...
void G2P5::SetWhitePoints(const std::vector<Vec2d> &pt2d, Keyframe::Ptr kf, G2P5MapPtr &map) {
    ...
    // 使用 lidar 在世界坐标系中的位置作为原点
    Vec3d orig = T_world_lidar.translation();
    ...
    for (int i = 0; i < 360; ++i) {
        ...
        Vec3d p_world = T_world_lidar * p_local;
...
bool G2P5::SaveMapPPM(const std::string& filename) {
    UL lock{newest_map_mutex_};
    if (newest_map_ == nullptr) {
        return false;
    }
    return newest_map_->SavePPM(filename);
}

bool G2P5::SaveMapPGM(const std::string& filename) {
    UL lock{newest_map_mutex_};
    if (newest_map_ == nullptr) {
        return false;
    }
    return newest_map_->SavePGM(filename);
}
