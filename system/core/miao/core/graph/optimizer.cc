...
void Optimizer::InitFromRaw(int level) {
    ...
    if (edges_.empty()) {
        AWARN << "Attempt to initialize an empty graph";
        return;
    }
...
void Optimizer::InitFromLast(int level) {
    ...
    for (const auto &v : vertices_) {
        if (v.second->Marginalized()) {
            AERROR << "should not use marginalization in incremental mode. ";
            return;
        }
    }
...
int Optimizer::Optimize(int iterations) {
    if (iv_map_.empty()) {
        AWARN << "0 vertices to Optimize, maybe forgot to call "
    ...
    ok = algorithm_->Init();
    if (!ok) {
        AERROR << "Error while initializing";
        return -1;
    }
    ...
    for (int i = 0; i < iterations && !Terminate() && ok; i++) {
        ...
        if (Verbose()) {
            AINFO << "iteration= " << i << "\t chi2= " << std::fixed << this_chi2
...
void Optimizer::Update(const double *update) {
    ...
    for (const auto &v : iv_map_) {
        ...
        // AINFO << "vert: " << v->GetId() << ", upd: " << upd.transpose();
...
bool Optimizer::AddVertex(std::shared_ptr<Vertex> v) {
    if (config_.incremental_mode_) {
        if (config_.max_vertex_size_ > 0 && vertices_.size() == config_.max_vertex_size_) {
            ...
            if (verbose_) {
                AINFO << "replacing new vertex " << v->GetId() << " with old " << v_remove->GetId();
            }
